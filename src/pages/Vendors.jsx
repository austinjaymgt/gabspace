...
export default function Vendors() {
  ...
  async function handleSave() {
    ...
    if (error) setError(error.message)
      ...
      setShowForm(false)
      setForm({ name: '', category: '', email: '', phone: '', rate: '' })
      fetchVendors()
    }
    setSaving(false)
  }
    ...
    return (
      <div style={styles.page}>
        <div style={styles.detailHeader}>
          <button onClick={() => setSelectedVendor(null)} style={styles.backBtn}>
            ← Back to vendors
          </button>
          ...
          {vendors.map(vendor => (
            ...
            >
              <div style={styles.vendorTop}>
                ...
                {vendor.category && (
                  <div style={styles.categoryBadge}>{vendor.category}</div>
                )}
              </div>
              <div style={styles.vendorName}>{vendor.name}</div>
              {vendor.email && (
                <div style={styles.vendorDetail}>{vendor.email}</div>
              )}
              {vendor.phone && (
                <div style={styles.vendorDetail}>{vendor.phone}</div>
              )}
              {vendor.rate && (
                <div style={styles.vendorRate}>
                  ${parseFloat(vendor.rate).toLocaleString()}
                </div>
              )}
            </div>
          ))}
