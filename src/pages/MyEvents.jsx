  ...
  function handlePrint() {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head>
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;padding-bottom:20px;border-bottom:2px solid #1A1A2E;">
        ...
        <div style="background:linear-gradient(135deg,#7C5CBF,#6B8F71);padding:8px 16px;border-radius:10px;color:white;font-family:'Syne',sans-serif;font-weight:800;font-size:16px;">curators</div>
      </div>
      ...
      <div style="margin-top:32px;padding-top:16px;border-top:1px solid #f0f0eb;font-size:11px;color:#8585A0;text-align:center;">Generated by curators · ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
  ...
  function generateHTML() {
    ...
    return `<!DOCTYPE html><html><head>
      ...
      <div style="background:#1A1A2E;border-radius:20px;padding:40px;margin-bottom:24px;position:relative;overflow:hidden;">
        ...
        <div style="position:relative;z-index:1;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;">
            <div style="background:linear-gradient(135deg,#7C5CBF,#6B8F71);padding:8px 18px;border-radius:10px;color:white;font-family:'Syne',sans-serif;font-weight:800;font-size:18px;">curators</div>
      ...
      <div style="text-align:center;padding-top:24px;border-top:1px solid rgba(0,0,0,0.08);margin-top:8px;">
        ...
        <div style="font-size:11px;color:#B0B0C0;margin-top:4px;">Generated with curators · ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
      </div>
      ...
      {activeTab === 'concept' && data.concept_data && (() => {
        ...
        return (
          <>
            ...
            <div style={card}>
              {c.venueConsiderations && <><div style={sectionLabel}>Venue & Space</div><p style={{ ...bodyText, marginBottom: '16px' }}>{c.venueConsiderations}</p><hr style={divider} /></>}
              {c.productionNotes && <><div style={sectionLabel}>Production Notes</div><p style={{ ...bodyText, marginBottom: '16px' }}>{c.productionNotes}</p><hr style={divider} /></>}
              {c.pressAndContent && <><div style={sectionLabel}>Press & Content Strategy</div><p style={bodyText}>{c.pressAndContent}</p></>}
            </div>
            ...
            <div style={card}>
              {c.vendorCategories?.length > 0 && <><div style={sectionLabel}>Vendor Categories</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '16px' }}>{c.vendorCategories.map((v, i) => <span key={i} style={{ ...tag, ...tagStyles[i % tagStyles.length] }}>{v}</span>)}</div><hr style={divider} /></>}
              {c.aestheticKeywords?.length > 0 && <><div style={sectionLabel}>Aesthetic Keywords</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '16px' }}>{c.aestheticKeywords.map((k, i) => <span key={i} style={{ ...tag, ...tagViolet }}>{k}</span>)}</div><hr style={divider} /></>}
              {c.budgetAllocation?.length > 0 && <><div style={sectionLabel}>Budget Allocation</div>{c.budgetAllocation.map((b, i) => (<div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '6px' }}><span style={{ color: '#3D3D5C' }}>{b.line}</span><span style={{ fontWeight: '600', color: '#7C5CBF' }}>{b.pct}</span></div>))}</>}
            </div>
          </>
        )
      })()}
