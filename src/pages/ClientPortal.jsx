...
export default function ClientPortal() {
  ...
  const totalComments = portals.reduce((sum, p) => {
    return sum + Object.values(p.commentsMap).reduce((s, comments) => s + comments.length, 0)
  }, 0)
      ...
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            {portals.length} active portal{portals.length !== 1 ? 's' : ''} · {totalComments} comment{totalComments !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
              ...
              <div key={portal.id} style={styles.portalCard}>
                ...
                <div style={styles.portalMeta}>
                  ...
                  <div style={styles.metaItem}>
                    ...
                    <span style={{
                      ...styles.metaValue,
                      color: totalPortalComments > 0 ? '#1D9E75' : '#1a1a1a'
                    }}>
                      {totalPortalComments}
                    </span>
                  </div>
                ...
                {showUpdateForm === portal.id && (
                  <div style={styles.updateForm}>
                    ...
                    <div style={styles.updateFormActions}>
                      <button onClick={() => setShowUpdateForm(null)} style={styles.cancelBtn}>
                        Cancel
                      </button>
                      <button
                              ...
                              {editingUpdate === update.id ? (
                                ...
                                <>
                                  <div style={styles.updateItemHeader}>
                                    <div>
                                      {update.title && (
                                        <div style={styles.updateItemTitle}>{update.title}</div>
                                      )}
                                    ...
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                      ...
                                      <button onClick={() => deleteUpdate(update.id)} style={styles.iconBtn}>
                                        ✕
                                      </button>
                                    </div>
                                  </div>
                                  ...
                                  {updateComments.length > 0 && (
                                    <div style={styles.commentsSection}>
                                      <div style={styles.commentsLabel}>
                                        💬 {updateComments.length} comment{updateComments.length !== 1 ? 's' : ''}
                                      </div>
...
const styles = {
  ...
  portalList: { display: 'flex', flexDirection: 'column', gap: '16px' },
  portalCard: { backgroundColor: '#fff', borderRadius: '12px', padding: '24px', border: '1px solid #f0f0eb' },
  portalCardTop: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' },
  portalClientInfo: { display: 'flex', alignItems: 'center', gap: '12px' },
  avatar: { width: '40px', height: '40px', borderRadius: '50%', backgroundColor: '#1D9E75', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: '600', flexShrink: 0 },
  ...
  copyBtn: { padding: '8px 14px', borderRadius: '8px', border: '1px solid #1D9E75', backgroundColor: '#fff', color: '#1D9E75', fontSize: '12px', fontWeight: '600', cursor: 'pointer' },
  postBtn: { padding: '8px 14px', borderRadius: '8px', border: 'none', backgroundColor: '#1D9E75', color: '#fff', fontSize: '12px', fontWeight: '600', cursor: 'pointer' },
  portalMeta: { display: 'flex', gap: '24px', padding: '14px 16px', backgroundColor: '#fafaf8', borderRadius: '8px', marginBottom: '16px' },
  ...
  metaValue: { fontSize: '13px', color: '#1a1a1a', fontWeight: '500' },
  updateForm: { display: 'flex', flexDirection: 'column', gap: '10px', backgroundColor: '#fafaf8', borderRadius: '10px', padding: '16px', marginBottom: '16px' },
  ...
  cancelBtn: { padding: '8px 14px', borderRadius: '8px', border: '1px solid #e0e0e0', backgroundColor: '#fff', color: '#666', fontSize: '13px', cursor: 'pointer' },
  saveBtn: { padding: '8px 14px', borderRadius: '8px', border: 'none', backgroundColor: '#1D9E75', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer' },
  updatesSection: { borderTop: '1px solid #f0f0eb', paddingTop: '16px' },
  toggleUpdatesBtn: { background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px', color: '#666', fontWeight: '600', padding: 0, display: 'flex', alignItems: 'center', gap: '8px' },
  commentBadge: { fontSize: '11px', backgroundColor: '#f0faf6', color: '#1D9E75', padding: '2px 8px', borderRadius: '20px', fontWeight: '600' },
  updatesList: { display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '12px' },
  updateItem: { backgroundColor: '#fafaf8', borderRadius: '10px', padding: '14px 16px' },
  updateItemHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' },
  ...
  reactionPill: { padding: '3px 8px', borderRadius: '20px', backgroundColor: '#f0faf6', border: '1px solid #1D9E75', fontSize: '12px', color: '#1D9E75', fontWeight: '500' },
  commentsSection: { borderTop: '1px solid #f0f0eb', paddingTop: '10px', marginTop: '8px', display: 'flex', flexDirection: 'column', gap: '8px' },
  commentsLabel: { fontSize: '11px', color: '#aaa', fontWeight: '600', textTransform: 'uppercase', marginBottom: '4px' },
  commentItem: { display: 'flex', gap: '8px', alignItems: 'flex-start' },
  commentAvatar: { width: '24px', height: '24px', borderRadius: '50%', backgroundColor: '#f0f0eb', color: '#666', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: '600', flexShrink: 0 },
