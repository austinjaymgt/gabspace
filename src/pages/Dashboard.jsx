...
flexShrink: 0,
      ...
      <div style={{ flex: 1, paddingBottom: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
          <div style={{ fontSize: t.fontSizes.xs, fontWeight: '600', color: type.color, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            {type.label}
          </div>
        ...
        <div style={{ fontSize: t.fontSizes.md, fontWeight: '600', color: t.colors.textPrimary, marginBottom: '4px', lineHeight: '1.3' }}>
          {item.title}
        </div>
        {item.subtitle && (
          <div style={{ fontSize: t.fontSizes.base, color: t.colors.textSecondary, lineHeight: '1.5' }}>
            {item.subtitle}
          </div>
        )}
        {item.meta && (
          <div style={{ marginTop: '8px', padding: '6px 12px', backgroundColor: type.bg, borderRadius: t.radius.md, fontSize: t.fontSizes.sm, color: type.color, fontWeight: '500', display: 'inline-block' }}>
            {item.meta}
          </div>
        )}
      </div>
...
export default function Dashboard({ session, onNavigate }) {
  ...
  async function fetchFeed() {
    ...
    const feedItems = [
      ...
      ...(invoices || []).map(inv => ({ type: 'invoice', title: inv.invoice_number || 'Invoice', subtitle: inv.clients?.name ? `billed to ${inv.clients.name}` : null, meta: `$${parseFloat(inv.total_amount || 0).toLocaleString()} · ${inv.status}`, created_at: inv.created_at, id: `invoice-${inv.id}` })),
  ...
  return (
    <div style={{ padding: '32px', fontFamily: t.fonts.sans }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <h2 style={{ fontSize: '26px', fontWeight: '700', color: t.colors.textPrimary, margin: '0 0 4px', letterSpacing: '-0.5px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '24px' }}>
        ...
        {/* Active Clients */}
        <div style={{ backgroundColor: t.colors.bgCard, borderRadius: t.radius.lg, padding: '24px', border: `1px solid ${t.colors.borderLight}`, borderTop: `3px solid ${t.colors.primary}` }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <div style={{ flex: 1, height: '4px', backgroundColor: t.colors.borderLight, borderRadius: '2px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${stats.totalClients > 0 ? (stats.activeClients / stats.totalClients) * 100 : 0}%`, backgroundColor: t.colors.primary, borderRadius: '2px' }} />
            </div>
            <span style={{ fontSize: t.fontSizes.xs, color: t.colors.textTertiary }}>{stats.totalClients} total</span>
          </div>
        ...
        {/* Active Projects */}
        <div style={{ backgroundColor: t.colors.bgCard, borderRadius: t.radius.lg, padding: '24px', border: `1px solid ${t.colors.borderLight}`, borderTop: '3px solid #4466cc' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
            <div>
          ...
          {stats.activeProjectsList?.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {stats.activeProjectsList?.map((project, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', backgroundColor: '#f0f4ff', borderRadius: t.radius.md }}>
                  <span style={{ fontSize: t.fontSizes.sm, fontWeight: '500', color: '#1e3a8a' }}>{project.title}</span>
                  <span style={{ fontSize: t.fontSizes.xs, color: '#4466cc' }}>{project.clients?.name || 'No client'}</span>
                </div>
              ))}
            </div>
          )}
          <button onClick={() => onNavigate('projects')} style={{ fontSize: t.fontSizes.xs, color: '#4466cc', background: 'none', border: 'none', cursor: 'pointer', fontFamily: t.fonts.sans, fontWeight: '600', marginTop: '12px', padding: 0 }}>
            View all →
          </button>
        </div>
        ...
        <div style={{ backgroundColor: t.colors.bgCard, borderRadius: t.radius.lg, padding: '24px', border: `1px solid ${t.colors.borderLight}`, borderTop: '3px solid #10B981' }}>
          ...
          <button onClick={() => onNavigate('invoices')} style={{ fontSize: t.fontSizes.xs, color: '#10B981', background: 'none', border: 'none', cursor: 'pointer', fontFamily: t.fonts.sans, fontWeight: '600', marginTop: '12px', padding: 0 }}>
            View invoices →
          </button>
        </div>
        ...
        <div style={{ backgroundColor: t.colors.bgCard, borderRadius: t.radius.lg, padding: '24px', border: `1px solid ${t.colors.borderLight}`, borderTop: '3px solid #8B5CF6' }}>
                ...
                return (
                  <div key={i} style={{ padding: '8px 10px', backgroundColor: '#F5F3FF', borderRadius: t.radius.md }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      ...
                      {campaign.platform && (
                        <span style={{ fontSize: t.fontSizes.xs, color: platformColors[campaign.platform] || '#888', fontWeight: '600' }}>{campaign.platform}</span>
                      )}
                    </div>
                    {budget > 0 && (
                      <>
                        <div style={{ height: '3px', backgroundColor: '#DDD6FE', borderRadius: '2px', overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${pct}%`, backgroundColor: pct > 90 ? '#EF4444' : '#8B5CF6', borderRadius: '2px' }} />
                        </div>
                        <div style={{ fontSize: t.fontSizes.xs, color: '#6D28D9', marginTop: '3px' }}>${spend.toLocaleString()} of ${budget.toLocaleString()}</div>
                      </>
                    )}
                  </div>
                )
          ...
          <button onClick={() => onNavigate('campaigns')} style={{ fontSize: t.fontSizes.xs, color: '#8B5CF6', background: 'none', border: 'none', cursor: 'pointer', fontFamily: t.fonts.sans, fontWeight: '600', marginTop: '12px', padding: 0 }}>
            View all →
          </button>
        </div>
      </div>
...
<div style={{
  ...
  margin: '24px 0 16px',
  paddingBottom: '12px',
  borderBottom: `2px solid ${t.colors.borderLight}`,
}}>
  <div>
    <h2 style={{
  ...
  <span style={{
    ...
    {feed.length} updates
      ...
      {/* Activity Feed */}
      <div style={{ backgroundColor: t.colors.bgCard, borderRadius: t.radius.lg, border: `1px solid ${t.colors.borderLight}`, padding: '24px', maxHeight: '420px', overflowY: 'auto' }}>
