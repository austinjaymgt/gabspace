...
export default function RunOfShow({ eventId, eventTitle, eventDate, venue, businessSpaceId }) {
  ...
  function handlePrint() {
    ...
    const rows = items.map(item => `
      <tr>
        <td style="padding:12px 16px;border-bottom:1px solid #F0F0F2;white-space:nowrap;font-weight:600;color:#7F5793;">${item.item_date ? `<div style="font-size:11px;color:#6B8F71;font-weight:700;margin-bottom:2px;">${formatDate(item.item_date)}</div>` : ''}${formatTime(item.start_time)} → ${formatTime(item.end_time)}</td>
        <td style="padding:12px 16px;border-bottom:1px solid #F0F0F2;font-weight:600;color:#414042;">${item.title}</td>
        <td style="padding:12px 16px;border-bottom:1px solid #F0F0F2;">${item.role_label ? `<span style="background:#F0EBF9;color:#7F5793;padding:3px 10px;border-radius:20px;font-size:12px;font-weight:600;">${item.role_label}</span>` : ''}</td>
        <td style="padding:12px 16px;border-bottom:1px solid #F0F0F2;color:#9CA3AF;font-size:13px;">${item.notes || ''}</td>
      </tr>`).join('')
    printWindow.document.write(`<!DOCTYPE html><html><head>
      <title>Run of Show — ${eventTitle}</title>
      <link href="https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=DM+Sans:wght@300;400;500;600&display=swap" rel="stylesheet"/>
      <style>* { box-sizing:border-box; margin:0; padding:0; } body { font-family:'DM Sans',sans-serif; color:#414042; background:#fff; padding:48px; } @media print { body { padding:24px; } }</style>
    </head><body>
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;padding-bottom:20px;border-bottom:2px solid #414042;">
        <div>
          <div style="font-size:11px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:#9CA3AF;margin-bottom:6px;">Run of Show</div>
          <h1 style="font-family:'Syne',sans-serif;font-size:28px;font-weight:800;color:#414042;letter-spacing:-0.5px;margin-bottom:4px;">${eventTitle}</h1>
          <div style="font-size:14px;color:#9CA3AF;">${eventDate ? formatDate(eventDate) : ''}${venue ? ` · ${venue}` : ''}</div>
        </div>
        <div style="background:linear-gradient(135deg,#7F5793,#6B8F71);padding:8px 16px;border-radius:10px;color:white;font-family:'Syne',sans-serif;font-weight:800;font-size:16px;">gabspace</div>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#F5F5F7;">
          <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:600;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1px solid #F0F0F2;">Time</th>
          <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:600;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1px solid #F0F0F2;">Item</th>
          <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:600;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1px solid #F0F0F2;">Owner</th>
          <th style="padding:10px 16px;text-align:left;font-size:11px;font-weight:600;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1px solid #F0F0F2;">Notes</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <div style="margin-top:32px;padding-top:16px;border-top:1px solid #F0F0F2;font-size:11px;color:#9CA3AF;text-align:center;">Generated by gabspace · ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
  ...
  return (
    <div style={{ backgroundColor: t.colors.bgCard, borderRadius: '14px', padding: '24px', border: `1px solid ${t.colors.borderLight}`, marginBottom: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <h3 style={{ fontSize: '15px', fontWeight: '700', color: t.colors.textPrimary, margin: 0, fontFamily: t.fonts.heading }}>Run of Show</h3>
          ...
          {items.map((item, index) => (
            <div key={item.id} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, paddingTop: '14px' }}>
                <div style={{ width: '10px', height: '10px', borderRadius: '50%', backgroundColor: t.colors.primary, flexShrink: 0 }} />
                ...
                <div style={{ backgroundColor: t.colors.bgHover, borderRadius: '10px', padding: '12px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: item.notes ? '4px' : '0', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '12px', fontWeight: '700', color: t.colors.primary, whiteSpace: 'nowrap' }}>{formatTime(item.start_time)} → {formatTime(item.end_time)}</span>
                      {item.role_label && <span style={{ fontSize: '11px', fontWeight: '600', backgroundColor: t.colors.primaryLight, color: t.colors.primary, padding: '2px 8px', borderRadius: '20px', whiteSpace: 'nowrap' }}>{item.role_label}</span>}
                    </div>
